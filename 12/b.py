...
for row in range(len(grid)):
    for column in range(len(grid[row])):
        if grid[row][column] == "a":
            a_elevations.append((row, column))
...
min_steps_from_a = float("inf")
for m in a_elevations:
    try:
        if min_steps[m] < min_steps_from_a:
            min_steps_from_a = min_steps[m] 
    except:
        continue
print(min_steps_from_a)